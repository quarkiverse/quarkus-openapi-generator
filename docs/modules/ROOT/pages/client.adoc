...
== Type, schema, import and name mappings
...
|Name Mapping
|`quarkus.openapi-generator.codegen.spec.[filename].name-mappings.[property_name]`
|`quarkus.openapi-generator.codegen.spec.my_spec_yml.name-mappings._type=customType` will rename the `_type` property to `customType` in the generated code.
|===

Note that these configuration properties are maps. For the type-mapping the keys are OAS data types and the values are Java types. For the name-mapping the keys are original property names and the values are the new names to use in the generated code.
...
Name mappings are useful to resolve naming conflicts, for example when an OpenAPI spec has properties with names that are Java reserved words or that cause duplicate fields (e.g., `type` and `_type` would both map to the same Java field `type`):

[source,properties]
----
quarkus.openapi-generator.codegen.spec.my_spec_yml.name-mappings._type=customType
----

It's also possible to only use a type mapping with a fully qualified name, for instance `quarkus.openapi-generator.codegen.spec.my_spec_yml.type-mappings.File=java.io.InputStream`. For more information and a list of all types see the OpenAPI generator documentation on https://openapi-generator.tech/docs/usage/#type-mappings-and-import-mappings[Type Mappings and Import Mappings] and https://openapi-generator.tech/docs/customization#schema-mapping[Schema mapping]. For more information on name mappings see https://openapi-generator.tech/docs/customization#name-mapping[Name mapping].

See the module https://github.com/quarkiverse/quarkus-openapi-generator/tree/main/integration-tests/type-mapping[type-mapping] for an example of how to use type, import and schema mappings. See the module https://github.com/quarkiverse/quarkus-openapi-generator/tree/main/integration-tests/name-mapping[name-mapping] for an example of how to use name mappings.
...
[[customize-cdi-scopes]]
== Customize CDI Scopes

include::./includes/customize-cdi-scopes.adoc[leveloffset=+1, opts=optional]

[[register-rest-client]]
== Registering the REST Client

include::./includes/register-rest-client.adoc[leveloffset=+1, opts=optional`]

[[generate-method-per-media-type]]
== Generate separate method per media type

include::./includes/method-per-media-type.adoc[leveloffset=+1, opts=optional]
...
include::./includes/initialize-empty-collections.adoc[leveloffset=+1, opts=optional]
...
Generated clients support the `x-multi-segment` vendor extension for path parameters that may contain slashes.

When a parameter is marked with `x-multi-segment: true`, the generated client keeps the Java parameter type and method shape unchanged, but switches the runtime encoding behavior so the value can span multiple URL segments. Ordinary string path parameters continue to be encoded safely by default.

See xref:x-multi-segment.adoc[Multi-Segment Path Parameters] for examples, limitations, and security considerations.
...
[[generate-model-for-usage-as-bean-param]]
== Generate Model for Usage as Bean Param or not
...
include::./includes/supported-vendor-extensions.adoc[opts=optional, leveloffset=+1]
