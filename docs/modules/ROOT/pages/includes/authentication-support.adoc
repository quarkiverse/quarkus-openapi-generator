...
NOTE: The `security_scheme_name` portion is a placeholder for the actual security scheme name used in your OpenAPI spec. If your application builds the name dynamically, make sure the resolved value is a concrete, sanitized config key and not the literal text `<security_scheme_name>`.
...
NOTE: By default, if the outgoing request already has an `Authorization` header set, its value is forwarded as the API key header to avoid duplicate headers. If you need to keep both the `Authorization` header (e.g. a JWT bearer token) and a static API key in a separate header, disable this behavior with:

[source,properties]
----
quarkus.openapi-generator.[filename].auth.[security_scheme_name].use-authorization-header-value=false
----
