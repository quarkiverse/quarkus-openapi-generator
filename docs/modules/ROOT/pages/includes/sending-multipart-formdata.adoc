...
When using RESTEasy Reactive, the `jakarta.ws.rs.BeanParam` annotation is added in front of the multipart parameter by default:
...
If you prefer to build the multipart body programmatically, enable the `resteasy-reactive-client-form` property for the spec. In that mode the generated method accepts `org.jboss.resteasy.reactive.client.api.ClientMultipartForm` instead of a generated multipart POJO.
...
public interface MultipartService {
  ...
  @Consumes(MediaType.MULTIPART_FORM_DATA)
  @Produces(MediaType.TEXT_PLAIN)
  String sendMultipartData(ClientMultipartForm dataParts);
...
* The `ClientMultipartForm` option is only available for RESTEasy Reactive clients and must be enabled explicitly with `resteasy-reactive-client-form=true`.
====
...
See the module https://github.com/quarkiverse/quarkus-openapi-generator/tree/main/integration-tests/multipart-request[multipart-request] for an example of how to use this feature with generated multipart POJOs, and https://github.com/quarkiverse/quarkus-openapi-generator/tree/main/client/integration-tests/multipart-client-form[multipart-client-form] for an example using `ClientMultipartForm`.
