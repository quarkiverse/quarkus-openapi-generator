...
Many configuration options can be set **globally** and apply to all OpenAPI specs at once. When a specific per-spec value is defined, it takes precedence over the global one. To set a global default, omit the `.spec.<filename>` segment from the property key.

For example, to set `return-response` globally:
...
And to override it for a specific spec:

[source,properties]
----
quarkus.openapi-generator.codegen.spec.petstore_json.return-response=true
----
...
Configuring `additional-model-type-annotations` will add all annotations to all the generated model files (extra details can be found in https://openapi-generator.tech/docs/generators/java/#config-options[OpenApi Generator Doc]).

You can add an annotation to a specific model file using the vendor extension `x-class-extra-annotation` (see https://openapi-generator.tech/docs/generators/java/#supported-vendor-extensions[Supported Vendor Extensions] for more information).
[source,json]
----
{
  "components": {
    "schemas":{
      "HelloModel": {
        "type": "object",
        "x-class-extra-annotation": "@AnAnnotation",
        "properties": {
          "id": {
            "type": "string"
          }
...
See the https://github.com/quarkiverse/quarkus-openapi-generator/tree/main/client/integration-tests[integration-tests] module for more information of how to use this extension. Please be advised that the extension is on experimental, early development stage.
