...
This documentation helps you to use Quarkus OpenAPI Generator Server with the Apicurio code generator.
To see how to use with OpenAPITools, please see the xref:openapitools.adoc[OpenAPITools] documentation.
...
include::./includes/server-apicurio-getting-started.adoc[leveloffset=+1, opts=optional]
...
Instead of editing the OpenAPI specification directly, you can configure the return type for an operation using the `operation-ids` map in `application.properties`.
The extension value will be automatically injected into the spec before code generation.

[cols="1,3",options="header"]
|===
| Property
| `quarkus.openapi.generator.server.operation-ids."<operationId>".return-type`
|===
...
If your hand-written domain classes clash with the generated model class names, you can append a suffix to every generated model (bean) class using the `model-name-suffix` property:

[source,properties]
----
quarkus.openapi.generator.server.model-name-suffix=Dto
----

With the configuration above, a `Pet` schema under `components/schemas` generates a `PetDto` class instead of `Pet`. The suffix is applied only to the model classes; the generated JAX-RS resource interfaces (e.g. `PetResource`) keep their original names.

The property can also be set per specification:

[source,properties]
----
quarkus.openapi.generator.server.spec.petstore_openapi_json.model-name-suffix=Dto
----

[NOTE]
====
This property is only applied for the `apicurio` generator.
====
...
NOTE: This feature requires `apicurio-codegen` version `1.2.9.Final` or later.
