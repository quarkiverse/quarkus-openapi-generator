...
@QuarkusTest
public class MultipleEquivalentConsumesTest {
    static Stream<Class<?>> imagesApiClasses() {
        return Stream.of(
                org.acme.json.multiple.equivalent.consumes.api.ImagesApi.class,
                org.acme.yaml.multiple.equivalent.consumes.api.ImagesApi.class);
    }

    @ParameterizedTest
    @MethodSource("imagesApiClasses")
    void imagesApiShouldNotContainDuplicateMediaTypes(Class<?> imagesApiClass) throws NoSuchMethodException {
    ...
    private static void assertImagesApiPerClass(Class<?> imagesApiClass) throws NoSuchMethodException {
        ...
        Map<String, String> expectedMethodSuffixPerMediaType = Map.of(
                // image/*+jpeg and image/jpeg have the same suffix => they are deduplicated
        ...
        for (Method m : updateMethods) {
            ...
            assertThat(mediaType)
                    .as("Consumes media type must not be image/*+json.")
                    .isNotEqualTo("image/*+json");
        }
    }
}
